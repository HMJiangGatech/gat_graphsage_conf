...
def load_reddit():
    ...
    train = list(rand_indices[15000:80000])
...
class _ECELossM(nn.Module):
    ...
    def forward(self, logits, labels, clss):
        ...
        predictions = torch.tensor([1 if item>0.5 else 0 for item in softmaxes.cpu().numpy()]).type( torch.cuda.LongTensor if torch.cuda.is_available() else torch.LongTensor )
        labels = Variable(torch.LongTensor(labels).type( torch.cuda.LongTensor if torch.cuda.is_available() else torch.LongTensor )).squeeze().data
